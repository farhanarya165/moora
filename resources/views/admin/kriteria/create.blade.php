    ...
    <div class="card-body">
        <form action="{{ route('admin.kriteria.store') }}" method="POST">
            ...
            <div class="mb-3">
                ...
                <input type="number" class="form-control @error('bobot') is-invalid @enderror" id="bobot" name="bobot" value="{{ old('bobot') }}" step="0.01" min="0" max="1" required>
                @error('bobot')
                    <div class="invalid-feedback">
                        {{ $message }}
                    </div>
            ...
            <div class="mb-3">
                ...
                <select class="form-select @error('jenis') is-invalid @enderror" id="jenis" name="jenis" required>
                    <option value="benefit" {{ old('jenis') == 'benefit' ? 'selected' : '' }}>Benefit (Nilai lebih tinggi lebih baik)</option>
                    <option value="cost" {{ old('jenis') == 'cost' ? 'selected' : '' }}>Cost (Nilai lebih rendah lebih baik)</option>
                </select>
                @error('jenis')
                    <div class="invalid-feedback">
                        {{ $message }}
