    ...
    <div class="card-body">
        <form action="{{ route('admin.kriteria.update', $kriteria->id) }}" method="POST">
            ...
            <div class="mb-3">
                ...
                <input type="number" class="form-control @error('bobot') is-invalid @enderror" id="bobot" name="bobot" value="{{ old('bobot', $kriteria->bobot) }}" step="0.01" min="0" max="1" required>
                @error('bobot')
                    <div class="invalid-feedback">
                        {{ $message }}
                    </div>
            ...
            <div class="mb-3">
                ...
                <select class="form-select @error('jenis') is-invalid @enderror" id="jenis" name="jenis" required>
                    <option value="benefit" {{ (old('jenis', $kriteria->jenis) == 'benefit') ? 'selected' : '' }}>Benefit (Nilai lebih tinggi lebih baik)</option>
                    <option value="cost" {{ (old('jenis', $kriteria->jenis) == 'cost') ? 'selected' : '' }}>Cost (Nilai lebih rendah lebih baik)</option>
                </select>
                @error('jenis')
                    <div class="invalid-feedback">
                        {{ $message }}
            ...
            <div class="d-grid gap-2 d-md-flex justify-content-md-end">
                <a href="{{ route('admin.kriteria.index') }}" class="btn btn-secondary me-md-2">Batal</a>
